...
This is a server side plugin inspired by [Carpet-Mod](https://github.com/gnembon/fabric-carpet) for Minecraft `1.20.x`, `1.21.x` and `26.x` and above.

> This fork adds Minecraft **26.2** support (Paper 26.2, Java 25, CommandAPI 12, adventure 5.x) plus new bot-friendly commands.
...
| /fp info      | Show the fake player state as JSON           | fakeplayer.command.info      | Machine-readable output, ideal for RCON bots                     |
| /fp scan      | Scan surroundings as JSON                   | fakeplayer.command.scan      | `/fp scan [name]` or `/fp scan <radius> [limit] [name]`          |
| /fp block     | Query a single block as JSON                | fakeplayer.command.block     | `/fp block <x> <y> <z> [name]`                                   |
| /fp highlight | Toggle colored debug render                  | fakeplayer.command.highlight | `/fp highlight [name]`; colored particle markers around the fake player, matching `/fp scan` colors |
...
| /fp move      | Move                                      | fakeplayer.command.move      | `/fp move <forward\|backward\|left\|right> [blocks] [name]`; without `blocks` it moves the default distance (1 second) |
...
## Bot / automation support (26.2 fork additions)

The following commands print compact **JSON** on one line, so an external program (e.g. an RCON bot) can parse them easily:

- `/fp info [name]` - fake player state: `pos`, `dimension`, `yaw`, `pitch`, `health`, `hunger`, `on_ground`, `sneaking`, `sprinting`, `held_item`, `tick`
- `/fp scan [name]` or `/fp scan <radius> [limit] [name]` - bounded surroundings: own `pos`, the block the fake player is looking at (`looking_at`), the block at its feet (`block_at_feet`), the block below (`block_below`), the total entity count (`entities_total`) and the **nearest** entities (up to `limit`, default 10) with `type`, `name`, `pos` and `distance`. Output is deliberately capped so it never dumps thousands of coordinates.
- `/fp block <x> <y> <z> [name]` - the block type at an exact position.

Every block/entity in the JSON output carries a `color` field (e.g. `"color":"#e74c3c"`). The color is derived deterministically from the block/entity id, so the text color always matches the world render.

**Colored debug render:** `/fp highlight [name]` toggles a particle render around the fake player for nearby players (within 48 blocks):
- gold ring on the fake player itself
- colored box outlines around the blocks it is looking at / standing in / standing on
- colored rings around nearby entities (same selection as `/fp scan`)

Players running `/fp scan` also get a color-coded chat summary next to the JSON line, using the same colors.

Movement can now be **precise**: `/fp move forward 5` walks exactly ~5 blocks horizontally (within ~±0.5 block, measured every tick) and stops by itself. Omitting the block count keeps the original behaviour (walk 1 second, 2 seconds while sprinting). `/fp stop` cancels a precise move early.

This is meant as a small backbone for manual bot programming on top of the plugin: query state with `info`/`scan`/`block`, act with `move`/`look`/`attack`/`mine`/`use`/`jump`/`cmd`.
